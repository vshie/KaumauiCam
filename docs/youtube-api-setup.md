...
3. Name it something you'll recognise (e.g. `Wailoa Cam`), organization can be left blank, click **Create**.
...
3. App name: `Wailoa Cam`; support email: your email; developer contact: your email. The other fields can be left blank.
...
6. Sign in with the Google account from step 1. You'll see an **"Unverified app"** warning — this is expected for your own Cloud project. Click **Advanced → Go to Wailoa Cam (unsafe)**. It is safe: it's *your* project.
...
- On the **first streaming slot of each HST calendar day**, the extension inserts a fresh broadcast titled with `{date}` substituted, binds it to a reusable `Wailoa Cam (auto-managed)` liveStream, and starts ffmpeg pushing to that stream's RTMP endpoint.
...
- **Do not delete** the `Wailoa Cam (auto-managed)` liveStream from YouTube Studio while the extension is running. The extension will recreate it if it's missing, but that changes the stream key and produces a brief noisy patch.
...
| First broadcast title has literal `{date}` in it | Template lost its placeholder | Set title template to `Wailoa Cam - {date}` and save |

## Where the state lives

Two files on the Pi's persistent volume (`/usr/blueos/extensions/wailoacam` on the host, `/app/data` inside the container):
